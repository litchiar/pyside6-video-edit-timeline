...
App.directive("tlScrollableTracks", function () {
  return {
    ...
    link: function (scope, element, attrs) {
       ...
       * Ctrl + wheel zooms around the cursor position, Shift + wheel pans.
       */
      element.on("wheel",function (e) {
        if (e.ctrlKey) {
          ...
          const MIN_SCALE = 1;
          const MAX_SCALE = 210;
          const wheelEvent = e.originalEvent || e;
          const currentScale = parseFloat(scope.project?.scale) || MIN_SCALE;
          const cursorX = wheelEvent.clientX || wheelEvent.pageX || 0;
          const direction = wheelEvent.deltaY * (e.shiftKey ? -1 : 1);
          const targetScale = direction > 0 ? currentScale * ZOOM_FACTOR : currentScale / ZOOM_FACTOR;
          const clampedScale = Math.max(MIN_SCALE, Math.min(MAX_SCALE, targetScale));

          if (Math.abs(clampedScale - currentScale) > 0.001) {
            scope.setScale(clampedScale, cursorX);
          }
        } else if (e.shiftKey) {
          e.preventDefault();
