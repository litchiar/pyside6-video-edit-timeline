...
App.controller("TimelineCtrl", function ($scope) {
  ...
  var playheadAnimationHandle = null;
  var playheadLastTimestamp = null;

  function cancelPlayheadAnimation() {
    if (playheadAnimationHandle !== null) {
      window.cancelAnimationFrame(playheadAnimationHandle);
      playheadAnimationHandle = null;
    }
    playheadLastTimestamp = null;
  }

  function playheadAnimationStep(timestamp) {
    if (!$scope.playhead_animating) {
    ...
    var deltaSeconds = Math.max(0, (timestamp - playheadLastTimestamp) / 1000.0);
    playheadLastTimestamp = timestamp;

    var timelineDuration = parseFloat($scope.project.duration) || 0;
    var nextPosition = $scope.project.playhead_position + deltaSeconds;

    if (timelineDuration > 0) {
      nextPosition = Math.min(nextPosition, timelineDuration);
      if (nextPosition >= timelineDuration) {
        $scope.playhead_animating = false;
      }
    }

    $scope.$applyAsync(function () {
      $scope.movePlayhead(nextPosition);
      $scope.previewFrame(nextPosition);
      if ($scope.enable_playhead_follow) {
        $scope.centerOnPlayhead();
      }
    });

    if ($scope.playhead_animating) {
      playheadAnimationHandle = window.requestAnimationFrame(playheadAnimationStep);
    } else {
      cancelPlayheadAnimation();
    }
  }

  $scope.playPlayhead = function (options) {
    options = options || {};
    var startAt = options.startAt;
    if (startAt !== undefined && startAt !== null && !isNaN(startAt)) {
      $scope.movePlayhead(parseFloat(startAt));
    }
...
$scope.updateLayerIndex = function () {
  ...
  $scope.getClipInlineStyle = function (clip, index) {
    if (!clip) {
      return {};
    }

    var start = parseFloat(clip.start);
    if (!isFinite(start)) {
      start = 0;
    }
    var end = parseFloat(clip.end);
    if (!isFinite(end)) {
    ...
    var width = Math.max(0, (end - start) * $scope.pixelsPerSecond);
    var left = parseFloat(clip.position);
    if (!isFinite(left)) {
      left = start;
    }
    left = left * $scope.pixelsPerSecond;

    var style = {
      width: width + "px",
      left: left + "px",
      top: $scope.getTrackTop(clip.layer) + "px",
      "z-index": $scope.getZindex(clip, 1000, index)
    };
    ...
    if (clip.text_color) {
      style.color = clip.text_color;
    }

    return style;
  };

  $scope.getTransitionInlineStyle = function (transition, index) {
    if (!transition) {
      return {};
    }

    var start = parseFloat(transition.start);
    if (!isFinite(start)) {
      start = 0;
    }
    var end = parseFloat(transition.end);
    if (!isFinite(end)) {
      end = start;
    }
    var width = Math.max(0, (end - start) * $scope.pixelsPerSecond);
    var left = parseFloat(transition.position);
    if (!isFinite(left)) {
      left = start;
    }
    left = left * $scope.pixelsPerSecond;
    ...
    return {
      width: width + "px",
      left: left + "px",
      top: $scope.getTrackTop(transition.layer) + "px",
      "z-index": $scope.getZindex(transition, 5000, index)
    };
  };

  $scope.getTrackInlineStyle = function (layer) {
    var style = {
      width: $scope.getTimelineWidth(0) + "px"
    };
  ...
  $scope.getTrackPanelStyle = function (layer) {
    var style = {};
    if (layer && layer.color) {
      style["background-color"] = layer.color;
    }
