...
function findTrackAtLocation(scope, mouseY) {
  ...
  var scrollTop = scrolling_tracks.scrollTop();
  ...
  for (var layer_index = 0; layer_index < scope.project.layers.length; layer_index++) {
    ...
    var track_elem = $("#track_" + layer.number);
    if (!track_elem.length) {
      continue;
    }

    var trackTop = track_elem.position().top + scrollTop;
    var trackHeight = track_elem.outerHeight();
    if (!trackHeight || trackHeight <= 0) {
      trackHeight = layer.height || 0;
    }
    if (!trackHeight) {
      continue;
    }

    var layerCenterY = trackTop + (trackHeight / 2);
    var distance = Math.abs(mouseY - layerCenterY);

    if (distance < minDistance) {
      minDistance = distance;
      closestTrack = layer;
      // Keep layer metrics up to date
      layer.y = trackTop;
      layer.height = trackHeight;
    }
  }
